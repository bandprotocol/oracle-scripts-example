...
// Outputs for the oracle scripts
#[derive(OBIEncode, OBISchema)]
struct Output {
    rates: Vec<u64>,
}

const EXCHANGE_COUNT: u64 = 4;
...
// Supported exchanges for each token
// eg. BTC => 111... means BraveNewCoin, CryptoCompare and CoinGecko are supported for BTC.
...
fn get_symbols_from_input(input: String) -> Vec<String> {
...
fn execute_impl(input: Input) -> Output {
    ...
    for (exchange_id, symbols) in exchange_map.iter() {
        ...
        let symbols_vec =
            get_symbols_from_input(get_ds_input(symbols.to_vec()));
